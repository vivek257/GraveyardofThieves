/**
 * Urre 2012
 * Event which gets called whenever HUD is active and visible, sends rendering calls to the connected chain of HUD kismet actions
 */
	...
	if (OutputLinks.Length > 0)
	{
		if (OutputLinks[0].Links.Length > 0)
		{
			for (i = 0; i < OutputLinks[0].Links.Length; ++i)
			{
				...
				{
					RenderObject.PlayerOwner = PlayerController(PlayerController);
					RenderObject.Render(Canvas);
				}
			}
		}
		CheckActivate(PlayerController(PlayerController), PlayerController(PlayerController), false);
	}
}
